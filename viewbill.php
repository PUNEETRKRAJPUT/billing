...
<h4>Note: Bill Amount = Total bill - Paid by now<br />&copy; 2023</h4>
<?php
include 'db.php';
$id =$_REQUEST['id'];
...
while($row = mysqli_fetch_array($result))
  {
	  $prev=$row['prev'];// amount just paid now
	  $pres=$row['pres'];// total amount
	  $price=$row['price'];// no use right now
	  $totalcons=$pres - $prev;
	  //$bill=$totalcons * $price;
	  $bill=$totalcons;//bill to be paid
  echo "<tr>";
  echo "<td>" . $row['id'] . "</td>";
  echo "<td>" . $prev . "</td>";
  echo "<td>" . $pres . "</td>";
